...
// Updated with: notifications page, wishlist page, pocket sub-pages,
// buyer messages, buyer shell expanded, all logouts → /login
...
// Notifications & Wishlist - using hide to avoid conflicts
...
// import 'package:gude_app/features/wallet/presentation/screens/received_money_screen.dart'; // Uncomment & fix path if file exists
...
// ── Buyer Features ────────────────────────────────────────────────
// BuyerMessagesPage with hide to prevent conflict with shell
...
// BuyerNavShell with hide to prevent conflict with buyer_messages_page.dart
...
class AppRouter {
  static final router = GoRouter(
    ...
    routes: [
      ...
      GoRoute(path: '/wallet/received', builder: (_, __) => const ReceivedMoneyScreen()), // Make sure this import exists above

      // Pocket as a full page with optional initial pocket index
      GoRoute(
        path: '/wallet/pockets',
        builder: (c, s) {
          final extra = s.extra as Map<String, dynamic>?;
          final index = extra?['index'] as int? ?? 0;
          return WalletPocketsPage(initialIndex: index);
        },
      ),
      ...
      // ════════════════════════════════════════════════════════════════
      // BUYER SHELL — expanded with messages tab
      // ════════════════════════════════════════════════════════════════
      ShellRoute(
        builder: (context, state, child) => BuyerNavShell(child: child),
        routes: [
          GoRoute(path: '/buyer/marketplace', builder: (c, s) => const MarketplacePage()),
          GoRoute(path: '/buyer/messages',    builder: (c, s) => const BuyerMessagesPage()),
          GoRoute(path: '/buyer/profile',     builder: (c, s) => const BuyerProfilePage()),
        ],
      ),
    ],
  );
